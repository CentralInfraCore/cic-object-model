...
# review.check is deliberately NOT in ci.gates.
#
# INV-046 gates a RELEASE, not a commit. Running it on every push would demand
# an external review of every intermediate tree, which is unmeetable, and a rule
# that cannot be met gets switched off — at which point it gates nothing. It
# runs where the rule applies: on a pull request into `main`.
review.check: ## Verify an external review record exists for this tree (INV-046)
	@python3 tools/release_subject.py review
